...
class QubitStateWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.active = False  # Aktivita qubitu

    def setState(self, basis, bit, active=True):
        """Nastavenie stavu qubitu"""
        self.basis = basis
        self.bit = bit
        self.active = active
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # Vykresľovanie aktívneho qubitu podľa bázy a stavu
        if self.active:
            color = QColor(228, 155, 15) if self.basis == 'X' else Qt.GlobalColor.green
            painter.setBrush(QBrush(color))

            # Vykresľovanie hornej polovice pre bit 0, dolnej pre bit 1
            if self.bit == 0:
...
class QuantumCommitment(QWidget):
    ...
    def resetProtocol(self):
        """Resetovanie stavu protokolu na začiatok."""
    ...
    def setupUI(self):
        ...
        participantsLayout.setContentsMargins(0, 0, 0, 10)

        aliceLabel = QLabel("ALICE")
        aliceLabel.setAlignment(Qt.AlignmentFlag.AlignLeft)
        aliceLabel.setStyleSheet(
            "font-weight: bold; color: blue; padding-left: 10px; margin-top: 40px;")
        ...
        bobLabel = QLabel("BOB")
        bobLabel.setAlignment(Qt.AlignmentFlag.AlignRight)
        bobLabel.setStyleSheet(
            "font-weight: bold; color: red; padding-right: 15px; margin-top: 40px;")
    ...
    def getRandomBits(self):
        """Získanie aktuálne nastavených náhodných bitov z UI."""
        randomBits = []
        for i, (bit0Radio, bit1Radio, bitGroup) in enumerate(self.randomBitRadios):
            # Explicitná kontrola vybraného tlačidla
    ...
    def updateUI(self):
        """Aktualizácia UI podľa aktuálneho stavu protokolu."""
    ...
    def nextStep(self):
        """Posunutie protokolu o krok vpred."""
        try:
            if self.step == 0:
                # Výber bitu Alice
                ...
                # Nastavenie náhodných bitov a príprava stavov
                ...
                # Blokovanie tlačidla ďalšieho kroku
    ...
    def resetUI(self):
        """Resetovanie protokolu a UI do počiatočného stavu."""
        self.resetProtocol()
