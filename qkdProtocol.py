...
class QKDProtocol(QWidget):
    def __init__(self):
        ...
        # Load the UI
        uic.loadUi('qkd_protocol.ui', self)
        ...
        # Find the main layout to add the title
        ...
        #self.use_custom_bits.stateChanged.connect(lambda state: self.custom_bits_input.setEnabled(bool(state)))
        # Nastavíme dynamické vlastnosti pre checkbox a textové pole
        # Vylepšené prepojenie checkboxu s textovým poľom
        ...
        # Timer, ktorý spúšťa posielanie fotónov (bitov)
        self.timer = QTimer()
        self.timer.timeout.connect(self.send_next_photon)

        # Timer pre samotnú animáciu fotónu
        ...
        # Uchovanie výsledného kľúča (bitov) po "presievaní"
        self.shared_key = []

        # Uloženie aktuálneho fotónu a jeho atribútov
        self.photon = None
        self.alice_bit = None
    ...
    def toggle_custom_bits_input(self, state):
        ...
        if state:
            ...
            self.custom_bits_input.setFocus()  # Nastaví focus na textové pole
        else:
    ...
    def setup_table(self):
        ...
        # Set header colors to black
        for col in range(8):
            item = self.table.horizontalHeaderItem(col)
            item.setForeground(QColor("black"))

    def draw_static_elements(self):
        """Vykreslí statické prvky scény podľa referenčného obrázku."""
        # Vyčistenie scény
        self.scene.clear()
    ...
    def draw_alice_area(self):
        """Kreslí oblasť pre Alicu s červeným rámčekom."""
        ...
        # Tabuľka fotónov (4 riadky)
        self.draw_photon_source()

    def draw_photon_source(self):
        """Kreslí tabuľku so zdrojom fotónov s rôznymi polarizáciami."""
        # Rozmery tabuľky
        table_width = 80
        table_height = 100
        ...
        for i, (bit, symbol, color) in enumerate(table_entries):
            ...
            # Vypočítať pozíciu po zohľadnení škálovania
    ...
    def draw_bob_area(self):
        """Kreslí oblasť pre Boba so zeleným rámčekom."""
    ...
    def draw_quantum_channel(self):
        """Kreslí kvantový kanál s rôznymi polarizáciami fotónov."""
        # Vypočítajte pozíciu pravého okraja tabuľky
        table_width = 80
    ...
    def draw_polarizers_and_detectors(self):
        """Kreslí polarizačné deliče a detektory podľa referenčného obrázku."""
        # Delič 50:50 v spodnej časti - otočený o 90° s popisom pod komponentom
        ...
        # Rotátor 45° nad deličom
        self.draw_rotator()

        # Polarizačný delič pre diagonálnu bázu (horný) - otočený o 90°
        ...
        # Polarizačný delič pre rektilineárnu bázu (vpravo) - normálna orientácia
        ...
        # K dolnému detektoru - vertikálna čiara nadol
    ...
    def draw_polarizer(self, x, y, label_text=None, rotate_diagonal=False, label_position="top"):
        """Kreslí polarizačný delič ako štvorec s diagonálnou čiarou."""
        ...
        # Diagonálna čiara - normálna alebo otočená o 90°
        if rotate_diagonal:
            # Otočená diagonála (z pravého horného do ľavého dolného rohu)
            ...
            # Štandardná diagonála (z ľavého horného do pravého dolného rohu)
            diagonal = QGraphicsLineItem(
                x - size / 2,
                y - size / 2,
                x + size / 2,
        ...
        if label_text:
            ...
            if label_position == "bottom":
                ...
                label.setPos(
                    ...
                    y + size / 2 + 5  # Umiestnenie pod komponentom
                )
            elif label_position == "top":
                # Popis nad komponentom
                label.setPos(
                    x - label.boundingRect().width() / 2,
                    y - size / 2 - label.boundingRect().height() - 5  # Umiestnenie nad komponentom
                )
            elif label_position == "left":
                # Popis naľavo od komponentu
                label.setPos(
                    x - size / 2 - label.boundingRect().width() - 5,  # Posun doľava
                    y - label.boundingRect().height() / 2  # Zarovnanie na stred výšky
                )
            elif label_position == "right":
                # Popis napravo od komponentu
                label.setPos(
                    x + size / 2 + 5,  # Posun doprava
                    y - label.boundingRect().height() / 2  # Zarovnanie na stred výšky
                )
    ...
    def draw_rotator(self):
        """Kreslí rotátor 45° ako žltý obdĺžnik."""
        # Rozmery rotátora
        width = 30
        height = 15
        ...
        label.setPos(
            self.rotator_pos.x() - width / 2 - label.boundingRect().width() - 5,  # Posun doľava
            self.rotator_pos.y() - label.boundingRect().height() / 2  # Zarovnanie na stred výšky
        )
        label.setScale(0.9)  # Zmenšenie textu

    def draw_detector(self, pos, bit_value, polarization_symbol, color, rotation=0, offset_x=0, offset_y=0,
                      vertical_spacing=0):
        """Kreslí detektor ako polkruh a umiestni bit + polarizačný symbol správne s dodatočnými posunmi."""
        # Upravená pozícia detektora bližšie k čiaram
        ...
        detector.setBrush(QBrush(QColor(200, 220, 255)))  # Svetlomodrá výplň

        # Použitie transformácie na otočenie a posun detektora
        transform = QTransform()
        transform.translate(pos.x(), pos.y())  # Posun na správne miesto
        transform.rotate(rotation)  # Rotácia detektora
        ...
        # **Umiestnenie textov podľa bitovej hodnoty**
        if bit_value == "0":
            # **Horizontálne zarovnanie (0 a symbol vedľa seba)**
            spacing = 5  # Medzera medzi textami
            bit_x = pos.x() - (bit_text.boundingRect().width() + polarization_text.boundingRect().width() + spacing) / 2
            bit_y = pos.y() - radius / 2 - bit_text.boundingRect().height() / 2  # Mierne hore

            polarization_x = bit_x + bit_text.boundingRect().width() + spacing
            polarization_y = bit_y  # Rovnaká výška

            # **Špecifický posun pre "0" a "↘"**
            if polarization_symbol == "↘":
                bit_y -= 2.5  # Posun hore
                polarization_y -= 2.5  # Posun hore

            # **Špecifický posun pre "0" a "→"**
            if polarization_symbol == "→":
                bit_y += 15  # Posun dole
                polarization_y += 15  # Posun dole


        else:

            # **Vertikálne zarovnanie (1 nad symbolom) s nastaviteľnou medzerou**

            bit_x = pos.x() - bit_text.boundingRect().width() / 2
            bit_y = pos.y() - radius / 2 - bit_text.boundingRect().height() / 2  # Mierne hore

            polarization_x = pos.x() - polarization_text.boundingRect().width() / 2
            polarization_y = bit_y + bit_text.boundingRect().height() + vertical_spacing  # Medzera medzi bitom a symbolom

            # **Špecifický posun pre "1" a "↗"**

            if polarization_symbol == "↗":
                bit_x += 5  # Posun doprava
                polarization_x += 5  # Posun doprava
    ...
    def draw_detector_label(self, x, y, text):
        """Kreslí popisok detektorov ako modrý box s textom a aplikuje špecifické posuny."""

        # **Špecifické posuny pre jednotlivé popisky**
        ...
        label.setScale(1)  # Mierne zväčšenie pre lepšiu viditeľnosť
        label.setBrush(QBrush(QColor("black")))  # Čierna farba textu
        ...
        label_bg.setBrush(QBrush(QColor(200, 220, 255)))  # Svetlomodrá výplň
        label_bg.setPen(QPen(QColor("black"), 2))  # Čierny okraj rámčeka
        ...
        # **Zmena poradia pridania do scény:**
        self.scene.addItem(label_bg)  # Najprv pozadie
        self.scene.addItem(label)  # Potom text, aby bol navrchu
    ...
    def create_photon(self, state):
        """Vytvorí fotón s daným stavom (polarizáciou)."""
    ...
    def animate_photon_step(self):
        """Animuje pohyb fotónu s pridaným ladením a bezpečnostnými overeniami."""
        try:
            if self.current_step >= self.animation_total_steps:
                ...
                if self.animation_phase == "to_beam_splitter":
                    ...
                    # Fotón dorazil k detektoru - koniec animácie
    ...
    def send_next_photon(self):
        """Odošle ďalší fotón z Alicinej sekvencie, bezpečnejšia verzia."""
        try:
            if self.current_bit < self.num_bits and not self.photon_in_flight:
                ...
                table_width = 80
                table_x = self.alice_area.x() + (self.alice_area.width() - table_width) / 2
                table_right_edge = table_x + table_width  # Presný pravý okraj tabuľky
                ...
                self.animation_phase = "to_beam_splitter"  # Najprv ideme k deliču 50:50
                self.current_step = 0
    ...
    def update_table(self, bit_index, alice_bit, alice_basis, bob_basis, bob_bit):
        """Aktualizuje tabuľku s výsledkami s upravenými farbami."""

        # Pomocná funkcia na vytvorenie bunky tabuľky s väčším fontom
        def create_table_item(text, color, is_bold=False, size=14):
            item = QTableWidgetItem(str(text))
            font = QFont()
            font.setPointSize(size)  # Zväčšenie fontu (hodnotu môžete upraviť)
            if is_bold:
                font.setBold(True)
            item.setFont(font)
    ...
    def start_simulation(self):
        """Resetuje a spúšťa simuláciu BB84 s pridaným ladením."""
        try:
