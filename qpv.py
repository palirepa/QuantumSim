...
class QuantumPositionVerification(QWidget):
    def __init__(self, parent=None):
        ...
        # NOVÉ: Skutočná poloha nečestného dôkazníka (odlišná od deklarovanej)
        ...
        # Definície krokov protokolu - upravené pre presnejšiu reprezentáciu
        self.protocol_steps = [
    ...
    def setup_ui(self):
        ...
        # NOVÁ ÚPRAVA: Ovládače vzdialeností - kombinované deklarované a skutočné
    ...
    def next_protocol_step(self):
        ...
        # Podľa režimu používame rôzne postupnosti krokov
        if self.mode == "dishonest_prover":
            self.next_dishonest_step()
        else:
            # Pôvodná logika pre poctivého dôkazníka
    ...
    def next_honest_step(self):
        ...
        if self.protocol_step == 1:
            # Deklarácia polohy dôkazníka
            #self.add_classical_message("1. Dôkazník → Overovatelia: Deklarácia polohy")
    ...
    def update_animation(self):
        ...
        if self.v1_signal_active:
            # Škálovanie času na základe vzdialenosti - väčšia vzdialenosť = dlhšia animácia
            if self.mode == "dishonest_prover":
                # Pre nečestného dôkazníka používame SKUTOČNÚ vzdialenosť
                distance_factor_v1 = self.actual_distance_v1_to_prover / 100.0  # Normalizujeme na základe 100km
            else:
                # Pre poctivého dôkazníka používame deklarovanú vzdialenosť
                distance_factor_v1 = self.distance_v1_to_prover / 100.0  # Normalizujeme na základe 100km

            animation_duration_v1 = min(5.0, max(1.0, 2.0 * distance_factor_v1))  # 2s pri 100km
        ...
        if self.v2_signal_active:
            ...
            if self.mode == "dishonest_prover":
                # Pre nečestného dôkazníka používame SKUTOČNÚ vzdialenosť
                distance_factor_v2 = self.actual_distance_prover_to_v2 / 100.0  # Normalizujeme na základe 100km
            else:
                # Pre poctivého dôkazníka používame deklarovanú vzdialenosť
                distance_factor_v2 = self.distance_prover_to_v2 / 100.0  # Normalizujeme na základe 100km

            animation_duration_v2 = min(5.0, max(1.0, 2.0 * distance_factor_v2))  # 2s pri 100km
        ...
        if self.response_active_v1:
            ...
            if self.mode == "dishonest_prover":
                # Používame SKUTOČNÚ vzdialenosť pre nečestného dôkazníka
    ...
    def verify_response(self):
        ...
        if self.expected_measurement is None:
            ...
            # Inak kontrolujeme, či výsledok zodpovedá očakávanému
    ...
    def calculate_qber(self):
        ...
        # Ak QBER presahuje prah, detekujeme podvod
    ...
    def get_current_state(self):
        ...
        # Zistíme, ktoré kroky sa majú zobraziť podľa režimu
        if self.mode == "single":
...
class QPVVisualizationWidget(QWidget):
    ...
    def draw_single_mode(self, painter, w, h):
        ...
        # NOVÁ ÚPRAVA: Jednoduché zobrazenie výsledkov overenia v jednom riadku
        if step >= 6:
        ...
        # Kreslenie hodnoty bázy merania - POSUNUTIE VYŠŠIE
        if step >= 2:
    ...
    def draw_dishonest_mode(self, painter, w, h):
        ...
        # Pozícia skutočného dôkazníka - vypočítame na základe skutočných vzdialeností
        ...
        # Vypočítame x pozíciu skutočného dôkazníka na základe pomeru vzdialeností
        actual_prover_x_ratio = act_dist_v1 / max(1, total_act_dist)  # Vyhýbame sa deleniu nulou
        ...
        # OPRAVA: Zväčšenie šírky popiskov a zmena pozície, aby boli viditeľné
        ...
        # Jednoduché zobrazenie výsledkov v jednom riadku - len pre režim podvodu
        if step >= 6 and self.mode == "dishonest_prover":
            # OPRAVA: Posunutie výsledkov nižšie pod text kroku
            result_y = 80  # Zvýšil som z 40 na 80, aby to bolo pod textom kroku
            ...
            # Kreslenie hlavného výsledku - posunuté nižšie
            painter.setPen(QPen(overall_color, 2))
            painter.setFont(QFont("Arial", 14, QFont.Weight.Bold))
            painter.drawText(10, result_y - 30, w - 20, 30, Qt.AlignmentFlag.AlignCenter, overall)
