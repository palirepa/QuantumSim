...
class BlochSphereWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # UPRAVENÉ: Zobrazujeme fázový rozdiel až po vytvorení previazania (v kroku 4, nie 3)
            ...
            # UPRAVENÉ: Zobrazujeme previazanie už od kroku 3, nie až od kroku 4
    ...
    def draw_state_vector(self, painter, cx, cy, radius, angle, color_name):
        ...
        # Nastavenie farby textu podľa toho, ktorú šípku kreslíme
...
class QuantumTimeSync(QWidget):
    ...
    def setup_ui(self):
        ...
        # Stavová / logovacia oblasť - znížená výška
        ...
        # Pre kompatibilitu s existujúcim kódom, ktorý môže volať metódy na log_area
    ...
    def next_protocol_step(self):
        ...
        if self.protocol_step == 1:  # Spustenie precesie
        ...
        elif self.protocol_step == 3:  # Príprava previazania (PRESUNUTÉ Z KROKU 4)
        ...
        elif self.protocol_step == 4:  # Detekcia fázového rozdielu (PRESUNUTÉ Z KROKU 3)
            ...
            if alice_statevector is not None and bob_statevector is not None:
                ...
                # Pre qubit v superpozícii môžeme extrahovať informácie o fáze
                # zo vzťahu medzi koeficientami
            ...
            # Použitie Qiskit na simuláciu merania
            try:
                ...
                result = job.result()
                # Pre túto vizualizáciu vlastne nepotrebujeme výsledok merania
    ...
    def simulate_quantum_state(self):
        """Vráti kvantový stav pre vizualizáciu (pre budúcu kvantovú simuláciu)"""
        try:
            # Pre Alicu
